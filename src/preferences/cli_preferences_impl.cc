...
namespace
{
    ...
    class CliValue : public Value
    {
    public:
        CliValue(std::map<std::string, std::string>* map, std::string key):
            _map(map),
            _key(std::move(key))
        {
        }
    ...
    public:
        ...
        std::set<std::unique_ptr<Value>> GetAll() const override
        {
            ...
            for (const auto& [key, value] : _values)
                result.insert(std::make_unique<CliValue>(
