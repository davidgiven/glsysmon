...
// wayland_dock_impl.cc / fallback_dock_impl.cc and are selected via DockCreate().
class Dock {
public:
...
// Injected into components that need a Dock; the backend is chosen from the
// active video driver and its preferences are taken from the injected
// Preferences.
...
// Backend entry points; used by GetDockComponent() so app.cc stays
// platform-agnostic.
