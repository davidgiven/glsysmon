...
namespace
{
    ...
    public:
        ...
        SDL_Window* CreateWindow() override
        {
            ...
            // shell role, so we can attach the layer-shell role before any
            // buffer is attached.
            ...
            SDL_SetNumberProperty(
                props, SDL_PROP_WINDOW_CREATE_WIDTH_NUMBER, size);
            SDL_SetNumberProperty(
                props, SDL_PROP_WINDOW_CREATE_HEIGHT_NUMBER, size);
            SDL_Window* window = SDL_CreateWindowWithProperties(props);
            SDL_DestroyProperties(props);
            if (window == nullptr)
                return nullptr;

            _display = static_cast<wl_display*>(
                SDL_GetPointerProperty(SDL_GetWindowProperties(window),
                    SDL_PROP_WINDOW_WAYLAND_DISPLAY_POINTER,
                    nullptr));
            _surface = static_cast<wl_surface*>(
                SDL_GetPointerProperty(SDL_GetWindowProperties(window),
            ...
            wl_output* output = static_cast<wl_output*>(
                SDL_GetPointerProperty(SDL_GetDisplayProperties(display_id),
                    SDL_PROP_DISPLAY_WAYLAND_WL_OUTPUT_POINTER,
                    nullptr));

            _layerSurface = zwlr_layer_shell_v1_get_layer_surface(_shell,
                _surface,
                output,
            ...
            zwlr_layer_surface_v1_add_listener(
                _layerSurface, &_layerSurfaceListener, this);

            zwlr_layer_surface_v1_set_anchor(
                _layerSurface, AnchorForSide(side));
            // size 0 on the unconstrained axis lets the compositor stretch the
            // dock.
            zwlr_layer_surface_v1_set_size(
                _layerSurface, static_cast<uint32_t>(size), 0);
        ...
        void PollWindow(SDL_Window* window) override
        {
            ...
            if (_pendingWidth == 0 || _pendingHeight == 0)
                return;
            // Keep the swapchain in sync with the compositor-assigned surface
            // size.
            SDL_SetWindowSize(window,
                static_cast<int>(_pendingWidth),
    ...
    private:
            ...
            uint32_t version)
        {
            ...
            {
                dock->_shell =
                    static_cast<zwlr_layer_shell_v1*>(wl_registry_bind(registry,
                        name,
    ...
    const zwlr_layer_surface_v1_listener
        WaylandDockImpl::_layerSurfaceListener = {
